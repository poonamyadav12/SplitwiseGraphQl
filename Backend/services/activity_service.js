...
export async function getActivitiesV2(parent, args) {
    try {
        const activities = await getActivitiesByUserIdV2(args.userId, args.options);
        console.log("Activities By User ID " + JSON.stringify(activities));
        return { __typename: "PaginatedActivityList", ...JSON.parse(JSON.stringify(activities)) }
    } catch (err) {
        console.log(err);
        return createSingleError('Unable to successfully get the Activities!');
    }
}
