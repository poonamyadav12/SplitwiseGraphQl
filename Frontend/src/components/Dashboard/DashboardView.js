...
import { getUserTransactions } from '../../graphql/query'
...
class DashboardView extends React.Component {
  ...
  async fetchData() {
    try {
      if (this.props.user) {
        const response = await client.query({ query: getUserTransactions, variables: { userId: this.props.user.email } });
        if (response.data.getUserTransactions.error) {
          this.props.errorAlert(response.data.getUserTransactions.error);
        } else {
          this.setState({
            transactions: response.data.getUserTransactions.transactions,
          });
        }
      }
    } catch (err) {
