...
import { client } from '../..';
import { getActivity } from '../../graphql/query';
...
class ActivityView extends React.Component {
    ...
    async fetchData(index = null, sortOrder = null, pageSize = null, groupName = null) {
        try {
            if (this.props.user) {
                let options = {
                    pageIndex: Number(index != null ? index : this.state.pageIndex),
                    pageSize: Number(pageSize ? pageSize : this.state.pageSize),
                    sortOrder: sortOrder || this.state.sortOrder,
                    groupName: groupName != null ? groupName : this.state.groupName
                }

                const response = await client.query({ query: getActivity, variables: { userId: this.props.user.email, options: options } });
                if (response.data.getActivity.error) {
                    this.props.errorAlert(response.data.getActivity.error);
                } else {
