...
import { client } from '../..';
import { getFriendTransactions } from '../../graphql/query';
    ...
    async fetchData() {
        try {
            if (this.props.friend) {
                const response = await client.query({ query: getFriendTransactions, variables: { friendId: this.props.friend.email, userId: this.props.user.email } })

                if (response.data.getFriendTransactions.error) {
                    this.props.errorAlert(response.data.getFriendTransactions.error);
                } else {
                    this.setState({
                        transactions: response.data.getFriendTransactions.transactions,
                    });
                }
            }
        } catch (err) {
