...
import { client } from '../..';
import { settleUpTransaction } from '../../graphql/mutation';
...
function SettleUpModal(props) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await client.mutate({ mutation: settleUpTransaction, variables: settlementTxns });
      if (response.data.settleUpTransaction.error) {
        setErrorMsg(response.data.settleUpTransaction.error);
      } else {
