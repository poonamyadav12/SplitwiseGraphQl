...
import { client } from '../..';
import { addComment, deleteComment } from '../../graphql/mutation';
...
class Comments extends React.Component {
    ...
    async handleSubmit() {
        ...
        try {
            const response = await client.mutate({ mutation: addComment, variables: data })

            if (response.data.addComment.error) {
                this.props.errorAlert(response.data.addComment.error);
            } else {
                this.setState({ transaction: response.data.addComment, comment: '' })
            }
        } catch (error) {
...
function CommentDeleteWarningModal(props) {
    async function handleDeleteSubmit() {
        ...
        try {
            const response = await client.mutate({ mutation: deleteComment, variables: { _id: props.comment._id } });
            if (response.data.deleteComment.error) {
                props.errorAlert(response.data.deleteComment.error);
            } else {
                props.hide();
            }
        } catch (error) {
...
function convertToUiTransactionView(backendTxn) {
    let transaction = { ...backendTxn }; // Need to allow extending the object
    ...
    transaction.to = transaction.to.map((payee) => {
        return { ...payee, oweAmount: amountPerMember };
    });
