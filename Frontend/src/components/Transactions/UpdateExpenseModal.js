...
import { updateTransaction } from '../../graphql/mutation';
...
function UpdateExpenseModal(props) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await client.mutate({ mutation: updateTransaction, variables: data.transaction })
      if (response.data.updateTransaction.error) {
        setErrorMsg(response.data.updateTransaction.error);
      } else {
