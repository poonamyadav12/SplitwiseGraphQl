...
import { client } from '../..';
import { createTransaction, joinGroupMutation, leaveGroupMutation } from '../../graphql/mutation';
import { getGroup, getGroupTransactions } from '../../graphql/query';
        ...
        try {
            if (this.props.groupId) {
                const [groupRes, txnRes] = await Promise.all([
                    client.query({ query: getGroup, variables: { id: this.props.groupId } }),
                    client.query({ query: getGroupTransactions, variables: { groupId: this.props.groupId } })
                ]);

                if (groupRes.data.getGroup.error || txnRes.data.getGroupTransactions.error) {
                    let err = groupRes.data.getGroup.error;
                    err.push(txnRes.data.getGroupTransactions.error);
                    this.props.errorAlert(err);
                } else {
                    this.setState({
                        group: groupRes.data.getGroup,
                        transactions: txnRes.data.getGroupTransactions.transactions,
                    });
                }
            }
        } catch (err) {
...
const GroupHeader = (props) => {
    ...
    async function leaveGroup() {
        ...
        try {
            const response = await client.mutate({ mutation: leaveGroupMutation, variables: data });
            if (response.data.leaveGroup.error) {
                props.errorAlert(response.data.leaveGroup.error);
            } else {
    ...
    async function joinGroup() {
        ...
        try {
            const response = await client.mutate({ mutation: joinGroupMutation, variables: data });
            if (response.data.joinGroup.error) {
                props.errorAlert(response.data.joinGroup.error);
            } else {
                props.refreshHomeData(props.user.email);
                props.reloadGroupView();
            }
...
function AddExpenseModal(props) {
    ...
    async function handleSubmit(e) {
        ...
        try {
            const response = await client.mutate({ mutation: createTransaction, variables: data.transaction });

            if (response.data.createTransaction.error) {
                setErrorMsg(response.data.createTransaction.error);
            } else {
                props.reloadGroupView();
                props.closeModal();
            }
