...
import { client } from '../..';
import '../../App.css';
import { createGroup, updateGroup } from '../../graphql/mutation';
...
function GroupCreateOrUpdateModal(props) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await client.mutate({ mutation: props.createMode ? createGroup : updateGroup, variables: group });
      if (props.createMode) {
        if (response.data.createGroup.error) {
          setErrorMsg(response.data.createGroup.error);
        } else {
          props.reloadViews();
          props.closeModal();
        }
      } else {
        if (response.data.updateGroup.error) {
          setErrorMsg(response.data.updateGroup.error);
        } else {
          props.reloadViews();
          props.closeModal();
        }
